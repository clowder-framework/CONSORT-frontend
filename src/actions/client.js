		...
		if (dataset_json !== undefined && dataset_json !== null) {
			...
			if (file_json !== undefined){
				...
				if (file.type == "text/plain"){
					const rct_extraction_submission = submitForExtraction(file_json.id, config.rct_extractor, clientInfo);
					...
					if (pdf_extraction_submission) {
						//const pdf_extraction_status = await checkExtractionStatusLoop(file_json.id, config.pdf_extractor, 5000, clientInfo);
